...
test('fixture exporter writes stable GNI handoff fixtures', async () => {
  ...
  try {
    ...
    const firebreakTrace = JSON.parse(await readFile(join(dir, 'gni_firebreak_trace_v1.json'), 'utf8'));
    ...
    assert.equal(firstListening.schema, 'FirstListeningRunV1');
    ...
    assert.equal(sessionShape.schema, 'SessionShapeSelectionV1');
    assert.equal(sessionShape.shapeId, 'dark_mirror');
    assert.equal(sessionShape.playerFacingText, null);
    ...
    assert.equal(sessionArc.schema, 'SessionArcV1');
    assert.deepEqual(sessionArc, fixtureRunSave.payload.sessionArc);
    ...
    assert.equal(firebreakTrace.schema, 'GniFirebreakTraceV1');
    assert.deepEqual(firebreakTrace, bridgeResult.firebreakTrace);
